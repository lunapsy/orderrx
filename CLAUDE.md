...
1. **며칠 실사용 후 덤프 JSON 리뷰** — 운영자 약국에서 2~3일 사용 후 "내 데이터 내보내기" 결과를 같이 살펴보고, 금지 토큰 스캔(18종) + 실제 워크플로 시퀀스가 의도대로 잡히는지 확인.
2. **테스터 모집 재검토** — 현재 보류 상태. 사전 준비(§7 준수 사항 반영) 완료 후 재개.
3. **M2 backend 설계 시작 준비** — 데이터 수집이 안정화되면 M2 진입. 단 §7의 약관·프라이버시 검토를 먼저 반영해야 함.
