...
- 2026-07-13: **0.3.1 — 빌드 채널 분리 (투트랙 배포 결정).** zip 직접 배포는 심사 제약을 받을 이유가 없다는 판단으로 두 채널로 분리: `pnpm ext:build` = **pilot** 채널(`<all_urls>` 정적 주입, 권한 프롬프트 없음, zip 배포용) / `pnpm ext:build:store` = **store** 채널(설치 시 host 권한 0개 + 도메인별 런타임 권한, 심사 제출용). 채널은 `ORDERRX_CHANNEL` 환경변수 → `__ORDERRX_STORE_BUILD__` define → `build_flags.ts`. pilot 빌드는 잔존 동적 등록을 자동 해제해 이중 주입을 방지 (같은 unpacked 폴더에 채널을 덮어쓰면 extension ID가 같아서 필요).
