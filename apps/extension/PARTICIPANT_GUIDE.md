...
2. **도메인 추가**: 본인이 사용하는 약국 주문 사이트의 호스트명을 입력합니다 (예: `example-order.co.kr`). 최대 30개까지 등록 가능합니다.
   - "추가"를 누르면 Chrome이 **"[사이트]의 데이터를 읽고 변경할 수 있음" 권한 요청 창**을 띄웁니다. **"허용"을 눌러야** 그 사이트에서 수집이 가능합니다 (v0.3.0부터 확장은 이렇게 허가된 사이트에만 접근하며, 그 외 사이트에는 아예 주입되지 않습니다). 실수로 거부했다면 도메인을 다시 추가하면 됩니다.
