...
**핵심**: 이 확장은 서버로 아무것도 업로드하지 않습니다. 모든 데이터는 **본인 브라우저 안에만** 저장됩니다. 참여자가 직접 JSON으로 내보내 운영자에게 전달해야 데이터가 운영자에게 도달합니다. 즉, 모든 공유는 명시적 의사결정을 거칩니다.
...
1. 운영자로부터 받은 `orderrx-extension-0.1.0.zip` 을 컴퓨터에 저장한 뒤 압축을 풉니다. 풀면 `orderrx-extension-0.1.0` 폴더가 생깁니다.
2. 이 폴더를 **안정적인 로컬 경로**에 둡니다. 예: `~/Tools/orderrx-extension-0.1.0/` (macOS), `C:\Tools\orderrx-extension-0.1.0\` (Windows).
...
2. 3.1에서 준비한 `orderrx-extension-0.1.0` 폴더를 선택 (안의 `manifest.json` 이 아니라 **폴더 자체**를 선택).
...
4. 운영자에게 "참여 철회" 알림 (이미 전달한 JSON 파일이 있다면 그 파일에 대해서도 삭제 요청 가능)
